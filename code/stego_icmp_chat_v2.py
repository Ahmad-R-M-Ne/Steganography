...
def check_connectivity(peer_ip: str, timeout: int = 2) -> bool:
    ...
    pkt = IP(dst=peer_ip) / ICMP(type=8, code=0) / Raw(load=b"CONNECTIVITY-CHECK")
...
def send_stego_message(session: ChatSession, text: str, delay: float = 0.03) -> None:
    ...
    send_icmp_packet(session.peer_ip, session.session_id, ICMP_START_SEQ, message_id)
    time.sleep(delay)

    for chunk_number, ip_id_value in chunks.items():
        send_icmp_packet(session.peer_ip, session.session_id, chunk_number, ip_id_value)
    ...
    send_icmp_packet(session.peer_ip, session.session_id, ICMP_END_SEQ, len(chunks))
...
def process_received_packet(session: ChatSession, pkt) -> None:
    ...
    if seq == ICMP_START_SEQ:
        message_id = ip_id
        session.received_buffers[message_id] = MessageBuffer(message_id=message_id)
    ...
    if seq == ICMP_END_SEQ:
        ...
        try:
            encrypted_blob = reassemble_chunks(buffer.chunks, buffer.total_chunks)
    ...
    if 0 < seq < ICMP_END_SEQ:
        ...
        message_id = max(session.received_buffers.keys())
        session.received_buffers[message_id].chunks[seq] = ip_id
...
def main() -> None:
    ...
    print(" ICMP Echo / IPv4 Identification / ChaCha20-Poly1305 / ACK")
    print("=" * 66)
    ...
    thread = threading.Thread(target=receiver_thread, args=(session,), daemon=True)
    thread.start()
